...
BoundingBox BoundingBox::boxUnion(const BoundingBox& b1, const BoundingBox& b2) {
    return { Vec3::minOfTwo(b1.minCorner, b2.minCorner), Vec3::maxOfTwo(b1.maxCorner, b2.maxCorner) };
}

BoundingBox BoundingBox::constructFromMesh(const Mesh& mesh) {
    return { Vec3::minOfTwo(Vec3::minOfTwo(mesh.a, mesh.b), mesh.c),
...
float BoundingBox::intersect(const Ray& ray) const {
    ...
    if ((tmin > tymax) || (tymin > tmax)) return std::numeric_limits<float>::max();
...
std::pair<std::vector<Object*>, std::vector<Object*>> BVH::splitObjects(std::vector<Object*> objects,
    ...
    if (maxExt == BoundingBox::Extent::x) {
        std::sort(objects.begin(), objects.end(),
                  [](const Object* lhs, const Object* rhs) { return lhs->box.centroid().x < rhs->box.centroid().x; });
    } else if (maxExt == BoundingBox::Extent::y) {
        std::sort(objects.begin(), objects.end(),
                  [](const Object* lhs, const Object* rhs) { return lhs->box.centroid().y < rhs->box.centroid().y; });
    } else {
        std::sort(objects.begin(), objects.end(),
                  [](const Object* lhs, const Object* rhs) { return lhs->box.centroid().z < rhs->box.centroid().z; });
    }
...
bool Mesh::isPointInsideMesh(const Vec3& point) const {
    ...
    auto isValid = [this](const Vec3& m, const Vec3& n) -> bool { return m.cross(n).dot(normal) > 0; };
